...
export class FabricAdapter implements BlockchainAdapter {
    public readonly network = 'hyperledger-fabric';

    async anchorHash(contractId: string, hash: string): Promise<BlockchainReceipt> {
        console.log(`[STUB] Fabric: anchoring ${contractId} with hash ${hash.substring(0, 16)}...`);
        return {
            tx_hash: `fabric-${'0'.repeat(58)}`,
            block_number: 0,
